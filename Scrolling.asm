...
;===================================================================================================
;  Routines for scrolling the screen. The way we'll be doing scrolling is by letting the
; player 'roam' within a boundry then 'pushing' the screen when they hit a boundry.
; This should make things a little easier as there will never be a point when scrolling is
; instantly reversed.
; Also given that the only way to exceed the top threshold should be by ladder/elevator, and
; the bottom by an excessive fall (death), we can give the feel of a 4/8 way scroller while
; only implementing 4 seperate one way scrollers - which will be a LOT simpler.
;
; Ideally I'd like to implement a 'resetting' system at least on the Y axis where once the
; character stops moving, scrolling continues till he is back in the 'home position' where
; a start is possible with a 3 frame lead up. This shouldn't be that noticable from a player
; standpoint
;
; NOTE : Like Sprite movement, these routines perform a ONE pixel scroll, to scroll more than one
;        pixel, they must be called multiple times.
;
;       Direction scrolled is referring to the direction the character is moving, not the actual
;       direction of the screen, so things might seem a little 'bass-ackwards'
;
; WARNING - this is a LONG file full of special case routines and unwound loops
;---------------------------------------------------------------------------------------------------
; Scroll Method:
...
;
; During testing I'm pretty sure there are 4 viable speeds for scrolling that can be accomplished
; (at my level of coding anyways) - 1/2 pixel , 1 pixel, 2 pixels and 3 pixels.
; that should make for decent variable speed scrolling and player acceleration at a later date
;
; Notes :
...
; NOTE: SCROLL_DIRECTION
; Direction scrolled is referring to the direction the character is moving
